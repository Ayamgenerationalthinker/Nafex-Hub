...
import { Eye, MessageCircle, ShoppingBag, TrendingUp, TrendingDown, Users, ArrowDownLeft, Clock } from "lucide-react";
...
export default function SellerPerformance() {
  ...
  const payoutTotals = useMemo(() => {
    const paidOut = (txns ?? []).reduce((sum, t) => {
      if (t.type !== "payout") return sum;
      if (t.status !== "success") return sum;
      return sum + Number(t.amount);
    }, 0);
    const pendingPayout = (txns ?? []).reduce((sum, t) => {
      if (t.type !== "payout") return sum;
      if (t.status === "pending") return sum + Number(t.amount);
      return sum;
    }, 0);
    return { paidOut, pendingPayout };
  }, [txns]);
        ...
        <Card>
          <CardContent className="p-5 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-2xl font-bold text-foreground">GHS {payoutTotals.paidOut.toFixed(2)}</p>
              <p className="text-xs text-muted-foreground mt-1">Successful payouts to your account</p>
            </div>
